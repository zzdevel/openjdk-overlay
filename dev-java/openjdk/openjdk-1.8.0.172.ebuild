...
jdk_project_aarch64_port="${jdk_project_base}" # aarch64-port

# set to blank if project does not have required version (or has no pre/suffix)
jdk_aarch32_port_suffix="-aarch32-180511"
...
IUSE="+cacerts doc source examples webstart nsplugin aarch32-port aarch64-port pax_kernel"
...
pkg_setup() {
    ...
    use aarch64-port && [ "${jdk_project_aarch64_port}" = "${jdk_project_base}" ] && die "${PN} version ${PV} is not available for aarch64 port"
